...
int main()
{
	...
	int ply,p,players,first;		/* variable first tracks which */
									/* player goes first or on odd turns */
	...
	if( players==1 )		/* only when playing the computer */
	{
		printf("A flip of the bitcoin says that ");
		first = rand()%2;	/* first equals 1 or 0 */
		if( first )			/* if 1, human goes first */
			puts("you go first\n");
		else				/* if 0, computer goes first */
	...
	while(ply<9)
	{
		...
		{
			if( ply%2==first )		/* determine who goes first */
			{
				p = computer(grid);
			}
			else
